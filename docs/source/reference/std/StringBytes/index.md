...
````{roto:method} StringBytes.get(self: StringBytes, idx: u64) -> Option[char]
Get the character at byte offset `idx`.
````

````{roto:method} StringBytes.len(self: StringBytes) -> u64
Get the length of the string in bytes.
````

````{roto:method} StringBytes.list(self: StringBytes) -> List[u8]
Returns the list of bytes of this string.
````

````{roto:method} StringBytes.slice(self: StringBytes, start: u64, end: u64) -> Option[String]
