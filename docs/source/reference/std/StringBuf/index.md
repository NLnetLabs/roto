...
````{roto:method} StringBuf.as_string(self: StringBuf) -> String
Get the underlying `String` of this `StringBuf`.
````

````{roto:method} StringBuf.from(s: String) -> StringBuf
Create a `StringBuf` with an initial `String`.
````

````{roto:method} StringBuf.new() -> StringBuf
Create a new empty `StringBuf`.
````

````{roto:method} StringBuf.push_char(self: StringBuf, c: char)
Add a `char` to the end of this `StringBuf`.
````

````{roto:method} StringBuf.push_string(self: StringBuf, s: String)
