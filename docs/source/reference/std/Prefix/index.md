...
````{roto:method} Prefix.addr(self: Prefix) -> IpAddr
Returns the IP address part of a prefix.
````

````{roto:method} Prefix.eq(self: Prefix, other: Prefix) -> bool
Check whether those prefixes are the same.
````

````{roto:method} Prefix.len(self: Prefix) -> u8
Returns the length part of a prefix.
````

````{roto:method} Prefix.max_addr(self: Prefix) -> IpAddr
Returns the largest address of the prefix.
````

````{roto:method} Prefix.min_addr(self: Prefix) -> IpAddr
Returns the smallest address of the prefix.

This is the same as `Prefix.addr`.
````

````{roto:method} Prefix.new(ip: IpAddr, len: u8) -> Prefix
...
````{roto:method} Prefix.to_string(self: Prefix) -> String
