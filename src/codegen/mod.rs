...
use std::{
    alloc::Layout, any::Any, collections::HashMap, ffi::c_void, fmt::Debug,
...
struct ModuleData {
    ...
    /// These constants are generated by the script, it's very important that
    /// they are dropped before the JITModule, because that contains the drop
    /// function necessary to drop these.
    _roto_constants: HashMap<ResolvedName, RotoConstant>,
    ...
    cranelift_jit: JITModuleWrapper,
}

struct JITModuleWrapper(ManuallyDrop<JITModule>);

impl Drop for JITModuleWrapper {
    fn drop(&mut self) {
        ...
        unsafe {
            let cranelift_jit = ManuallyDrop::take(&mut self.0);
            cranelift_jit.free_memory();
        }
    }
}

impl std::ops::Deref for JITModuleWrapper {
...
impl RotoConstant {
    fn new(size: usize, align: usize, drop_fn: DropFn) -> Self {
    ...
    for item in ir {
        match &item.kind {
            ItemKind::Constant {
                ...
                let func_id =
                    module.define_function(item, &mut builder_context);
                module.inner.finalize_definitions().unwrap();

                let id = module
                    .functions
                    .get(&format!("::generated::drop_{type_id}"))
                    .unwrap()
                    .id;

                let drop_ptr = module.inner.get_finalized_function(id);
                let drop_ptr = unsafe {
                    std::mem::transmute::<*const u8, DropFn>(drop_ptr)
                };
...
impl ModuleBuilder {
    fn declare_constant(&mut self, constant: &RuntimeConstant) {
        ...
        self.runtime_constants
            .insert(constant.name, constant.value.clone());
    }
        ...
        let func_id = match &kind {
            ItemKind::Constant { .. } => {
                ...
                sig.params.push(AbiParam::new(
                    self.cranelift_type(&IrType::Pointer),
                ));
            ...
            ItemKind::Function { .. } => self.functions[name.as_str()].id,
        };
...
impl<'c> FuncGen<'c> {
    ...
    fn instruction(&mut self, instruction: &lir::Instruction) {
        match instruction {
            ...
            lir::Instruction::ConstantAddress { to, name } => {
                ...
                let ptr = if let Some(const_ptr) =
                    self.module.runtime_constants.get(name)
                {
                    const_ptr.ptr()
                } else if let Some(roto_constant) =
                    self.module.roto_constants.get(name)
                {
                    roto_constant.ptr
                } else {
                    ice!("Constant not defined")
                };

                let val = self.ins().iconst(ty, ptr as usize as i64);
    ...
    fn operand(&mut self, val: &Operand) -> (ir::Value, Type) {
        match val {
            lir::Operand::Place(p) => {
                if p.kind == VarKind::Context
                    && !self.module.variable_map.contains_key(p)
                {
                    let ty = self.module.isa.pointer_type();
                    return (self.ins().iconst(ty, 0), ty);
                }
