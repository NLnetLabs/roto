...
pub trait RotoFunc {
    ...
    /// # Safety
    ///
    /// This function is highly unsafe as it will just call the function behind
    /// `func_ptr` with whatever values this function is given, with assumptions
    /// about the ABI.
    ///
    /// - The `func_ptr` must be a pointer to a function generated by Roto or
    ///   with the same ABI as a Roto function.
    /// - The context should match the context expected by the function behind
    ///   `func_ptr`.
    /// - The arguments must match the arguments expected by the function behind
    ///   `func_ptr`.
    /// - `return_by_ref` must accurately reflect whether the function behind
    ///   `func_ptr` returns a value via the first parameter.
    ///
