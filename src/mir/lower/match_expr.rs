...
use crate::{
    ...
    ice,
    label::LabelRef,
    mir::{Place, Projection, Ty, TyRef, Value},
...
impl Lowerer<'_> {
    ...
    pub fn r#match(&mut self, id: MetaId, m: &Meta<Match>) -> Value {
        ...
        let examinee_ty_ref = {
            ...
            self.type_info.convert(&examinee_ty)
        };
        let examinee_ty = self.type_info.ty_pool.get(examinee_ty_ref).clone();

        let Ty::Enum(variants) = examinee_ty else {
        ...
        let examinee = self.assign_to_var(examinee, examinee_ty_ref);
        ...
        for (discriminant, arm, arm_index) in branches {
        ...
        for (i, (_, arm, arm_index)) in branches.iter().enumerate() {
            ...
            } = &arm.pattern.node
            {
                    ...
                    fields.iter().zip(&variant.1).enumerate()
                {
                    ...
                    self.add_live_variable(var.clone(), field_ty);
                    self.do_assign(
                        Place::new(var, field_ty),
                        field_ty,
                        Value::Clone(Place {
                            var: examinee.clone(),
                            root_ty: examinee_ty,
            ...
            if let Some(guard) = &arm.guard {
                ...
                let op = self.assign_to_var(op, TyRef::BOOL);
