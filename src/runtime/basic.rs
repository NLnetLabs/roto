...
use crate::{
    macros::item,
    runtime::items::{Function, Item, Type},
};
...
fn ip_addr_methods() -> Vec<Function> {
    vec![
        item! {
...
fn string_methods() -> Vec<Function> {
    vec![
        item! {
...
pub fn built_ins() -> Vec<Item> {
    let mut bool_ty = item! {
        /// The boolean type
        ///
